...
internal signal (AUROC $0.897$: authorized deletes project higher than erroneous ones), but the two
...
deletes score higher, AUROC $=0.897$ for separating the two sets --- but the distributions \emph{overlap}
...
errors. The probe is rank-separable (AUROC $0.897$) but operationally blind.}
\label{fig:dist}
\end{figure}
...
repetition, but a conservative read at $n{=}14$ still gives a fail-rate lower bound $\approx 0.78$, and the
